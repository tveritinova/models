...
for folder in ['Set1Part0', 'Set2Part0']:
    #print(folder)
    imgs += parse_annotation('/jet/prs/swedish_signs/annotations_'+folder+'.txt', 
...
if train:
    imgs = np.random.choice(imgs, int(len(imgs) * 0.75))
else:
    imgs = np.random.choice(imgs, int(len(imgs) * 0.25))
...
for data_and_label_info in imgs:
